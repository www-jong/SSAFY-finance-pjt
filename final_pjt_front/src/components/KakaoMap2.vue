<template>
    <div class="map_wrap relative w-full h-screen">
        <!-- 지도 -->
        <div id="map" class="border border-black-300 shadow-2xl rounded-2xl"></div>

        <!-- 검색 옵션 및 결과 목록 -->
        <div id="menu_wrap"
            class="absolute top-0 left-0 bottom-0 w-90 m-2 p-2 overflow-y-auto bg-white rounded shadow-lg z-10">
            <div class="option">
                <select v-model="selectedSido" class="w-full mb-2 p-2 bg-gray-200 h-6 rounded">
                    <option disabled value="">시도를 선택하세요</option>
                    <option v-for="(value, name) in city_data" :key="name" :value="name">{{ name }}</option>
                </select>
                <select v-model="selectedSigungu" :disabled="!selectedSido" class="w-full mb-2 p-2 bg-gray-200 h-6 rounded">
                ...
                <select v-model="selectedEupmyeondong" :disabled="!selectedSigungu"
                    class="w-full mb-2 p-2 h-6 bg-gray-200 rounded">
